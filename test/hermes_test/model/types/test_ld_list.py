...
def test_build_in_get():
    ...
    assert li[:2] == ["foo", "bar"] and li[1:-1] == ["bar"]  # FIXME: maybe ld_list instead?
...
def test_build_in_set():
    ...
    li[0] = ld_list.from_list([{"@type": "schema:Thing", "schema:name": "a"}], parent=li, key=li.key, container="@set")
