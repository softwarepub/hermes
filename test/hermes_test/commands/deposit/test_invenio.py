...
def test_resolve_doi(requests_mock):
    requests_mock.get('https://doi.org/123.45/foo.bar-6789', status_code=302, headers={'Location': 'https://foo.bar/record/6789'})
...
def test_resolve_doi_wrong_host(requests_mock):
    requests_mock.get('https://doi.org/123.45/foo.bar-6789', status_code=302, headers={'Location': 'https://foo.baz/record/6789'})
...
def test_resolve_doi_unknown(requests_mock):
    requests_mock.get('https://doi.org/123.45/foo.bar-6789', status_code=302, headers={'Location': 'https://datacite.org/404.html'})
    requests_mock.get('https://datacite.org/404.html', status_code=200) # To show how broken datacite is right now
...
def test_resolve_record_id(requests_mock):
    requests_mock.get('https://foo.bar/api/records/6789', text='{"links":{"latest":"https://foo.bar/api/records/12345"}}')
